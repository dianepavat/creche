...
if (length(keep_ud_ids) == 0) {
  stop(paste("No successful AKDEs for dataset:", dataset_name))
}
  

# ---------------------------------------------------------------
# Re-run AKDE on the successful bird-years as a LIST so all UDs
# are created on the same grid and can be averaged safely
# ---------------------------------------------------------------
cat("Re-running AKDE on shared grid for year-level averaging...\n")

tele_list_shared <- tele_list_ok[keep_ud_ids]
fit_list_shared <- fit_list_ok[keep_ud_ids]

if (is.null(water_boundary_sp)) {
  ud_list_shared <- ctmm::akde(
    tele_list_shared,
    fit_list_shared,
    weights = use_weighted_akde,
    debias = use_debiased_akde
...
names(ud_list_shared) <- names(tele_list_shared)

# ---------------------------------------------------------------
# Summarise individual bird-year areas from the shared-grid UDs
# ---------------------------------------------------------------
individual_area_tbl <- purrr::map_dfr(names(ud_list_shared), function(id) {
  ...
  out <- purrr::map_dfr(ud_levels, function(lvl) {
    extract_ud_area(ud_list_shared[[id]], level_ud = lvl)
  })
...
bird_year_tbl_ok <- tracks_use %>%
  ...
  filter(ctmm_id %in% names(ud_list_shared))

year_ud_list <- split(
  ud_list_shared,
  bird_year_tbl_ok$year_season[match(names(ud_list_shared), bird_year_tbl_ok$ctmm_id)]
)
...
year_ud_list <- lapply(year_ud_list, function(x) {
  ...
  saveRDS(fit_list_shared, file.path(dataset_dir, "ctmm_fit_list.rds"))
  saveRDS(ud_list_shared, file.path(dataset_dir, "ctmm_ud_list.rds"))
